...
ts_t     ts_map32(u_int32_t freq, u_int32_t ts32);
...
ts_t     ts_mul      (ts_t ts,  u_int32_t x);
ts_t     ts_div      (ts_t ts,  u_int32_t x);
...
ts_t     ts_convert(u_int32_t new_freq, ts_t ts);

/* Conversion to milliseconds */
u_int32_t  ts_to_ms(ts_t t1);

/* Conversion to microseconds */
u_int32_t  ts_to_us(ts_t t1);

/* Debugging functions */
int      ts_valid(ts_t t1);
u_int32_t  ts_get_freq(ts_t t1);
...
ts_t    ts_seq32_in  (ts_sequencer *s, u_int32_t f, u_int32_t curr_32);
u_int32_t ts_seq32_out (ts_sequencer *s, u_int32_t f, ts_t    curr_ts);
