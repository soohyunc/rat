...
int
layered_encoder_create(u_char **state, u_int32 *len)
{
...
void
layered_encoder_destroy(u_char **state, u_int32 len)
{
...
int
layered_encoder_get_parameters(u_char *state, char *cmd, u_int32 cmd_len)
{
...
int
layered_decoder_peek(u_int8   pkt_pt,
                     u_char  *buf,
                     u_int32  len,
                     u_int16  *upp,
                     u_int8   *pt)
...
/*                debug_msg("pt = %d, mrk = %d, len = %d, totlen = %d\n", hdrpt, mrk, blen, totlen);
                ...
                data += 4 + blen;
                hdr32 = ntohl(*(u_int32*)p);
/*                assert(((u_int32)data - (u_int32)buf) <= blen); */
        }
        else {
                debug_msg("Invalid layered header\n");
                goto fail;
 ...
 */
int 
layered_decoder_describe (u_int8   pkt_pt,
                          u_char  *data,
                          u_int32  data_len,
                          char    *out,
                          u_int32  out_len)
